...
AS 
SELECT * FROM tbl_books WHERE title = 'The Lost Tribe'
GO
...
CREATE PROCEDURE SharpstownLostTribe
AS
SELECT * FROM tbl_book_copies WHERE book_id = 21 AND branch_id = 1
GO

EXEC SharpstownLostTribe

/* Question 2 */
CREATE PROCEDURE LostTribeTotal
AS
SELECT * FROM tbl_book_copies WHERE book_id =21 and branch_id BETWEEN 1 AND 4
GO
...
AS
SELECT tbl_borrower.borrower_name, tbl_borrower.card_no, tbl_book_loans.card_no, tbl_book_loans.book_id
...
CREATE PROCEDURE WhoBorrowed
AS
SELECT tbl_book_loans.branch_id, tbl_book_loans.date_due, tbl_borrower.borrower_name, tbl_borrower.borrower_address, tbl_books.title
FROM tbl_book_loans
INNER JOIN tbl_borrower ON tbl_borrower.borrower_address = tbl_borrower.borrower_address AND tbl_borrower.borrower_name = tbl_borrower.borrower_name
INNER JOIN tbl_books ON tbl_books.title = tbl_books.title
Where branch_id = 1 AND date_due = 2142020
GO
